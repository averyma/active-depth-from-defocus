...
net_path  = ['C:\Users\avery\Desktop\DfD\20170412ScreenCap\r\train234_test1\convnet_r.mat'];
load(net_path)
folder = ['C:\Users\avery\Desktop\DfD\20170412ScreenCap\r\train234_test1\test1\'];
...
%     filename = files(i).name;
    ...
    if target_depth == output_depth
        % if same, green
        class_map(centroids(j,2)-patch_size/2+1:centroids(j,2)+patch_size/2,...
                centroids(j,1)-patch_size/2+1:centroids(j,1)+patch_size/2,2) = 1;
    elseif abs(target_depth-output_depth)==0.5
        % if off by 0.5cm, cyan
        class_map(centroids(j,2)-patch_size/2+1:centroids(j,2)+patch_size/2,...
                  centroids(j,1)-patch_size/2+1:centroids(j,1)+patch_size/2,2:3) = 1;
    elseif abs(target_depth-output_depth)== 1
        % if off by 1cm, yellow
        class_map(centroids(j,2)-patch_size/2+1:centroids(j,2)+patch_size/2,...
                  centroids(j,1)-patch_size/2+1:centroids(j,1)+patch_size/2,1:2) = 1;
    else
        % if off by 1cm+, red
        class_map(centroids(j,2)-patch_size/2+1:centroids(j,2)+patch_size/2,...
                  centroids(j,1)-patch_size/2+1:centroids(j,1)+patch_size/2,1) = 1;
    end
    ...
    imwrite(class_map,['C:\Users\avery\Desktop\DfD\20170412ScreenCap\r\train234_test1\class_map\test1\' filename(1:3) '.png' ]);
end
